...
#go to https://www.mounriver.com/download to get 230 IDE.
...
windowsIdeFile = "MounRiver_Studio_Setup_V230.zip"
macosIdeFile = "MounRiver_Studio_MacOS_ARM64_V230.tar.gz"
linuxIdeFile = "MounRiverStudio_Linux_X64_V230.tar.xz"
...
if os.path.exists(extractedWindowsPack):
    ...
    #check if MounRiver_Studio_Setup_V230.exe is in the zip file
    listing = subprocess.check_output(["unzip", "-l", windowsfileDownloadedPath], text=True)
    if not "MounRiver_Studio_Setup_V230.exe" in listing:
        exit("MounRiver_Studio_Setup_V230.exe not found in the zip file, please check the zip file.")
    ...
    targetExePath = os.path.join(expandedIdePath, "MounRiver_Studio_Setup_V230.exe")
    if not os.path.exists(targetExePath):
        extractCommand = f'unzip -o "{windowsfileDownloadedPath}" "MounRiver_Studio_Setup_V230.exe" -d "{expandedIdePath}"'
...
extractedWindowsExe = os.path.join(originalIdePath, "expanded/MounRiver_Studio_Setup_V230.exe")
...
if doExtract:
    ...
    linuxOpenOcdPath = "MRS2/MRS-linux-x64/resources/app/resources/linux/components/WCH/OpenOCD/OpenOCD/"
...
if doExtract:
    ...
    extractCommand = f'tar --strip-components=9 -xJf "{linuxfileDownloadedPath}" -C "{extractedLinuxPack}" "MRS2/MRS-linux-x64/resources/app/resources/linux/components/WCH/Toolchain/RISC-V Embedded GCC12/"'
    extractResult = os.system(extractCommand)
    if extractResult != 0:
